...
graph twoway (rcap  ul ll n, lwidth(*1.9) msize(*1.5) horizontal lcolor(maroon*0.88) name(fig8,replace)) || (dot coef n, horizontal xtitle("(EUR/MWh)", size(*1.4)) mcolor(maroon%80) msymbol(d)  msize(*2.05) ytitle("") title("",position(11) size(*0.95)) barw(1.3) legend(off) ylabel(,valuelabel labsize(*1.4) angle(horizontal)) xlabel(,labsize(*1.35)) xline(0, lwidth(*1.9))) 

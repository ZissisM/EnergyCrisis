...
cap egen load_tgm=mean(load),by(hour)
...
graph export "fig1d_highres.svg", as(svg) width(3000) replace
...
grc1leg PLlevf4 PLsource2f4, altshrink legendfrom(PLsource2f4) name(PLg,replace) title("E)					Poland",size(*1.4) position(11)) rows(1) position(6) iscale(1)

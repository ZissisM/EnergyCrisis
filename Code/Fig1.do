 ...
 **this one used to generate the panel
 graph dot decarb_2021 decarb_2016,over(labels,sort(order) label(labsize(*0.92))) marker(1,msymbol(d) mcolor(maroon*0.7) msize(*1.8)) marker(2,mcolor(ebblue*0.8) msize(*2))  graphregion(margin(l+18)) title("A)", position(11) size(*1.35)) legend(label(1 "Decarbonized Energy Share 2021") label(2 "Decarbonized Energy Share 2016") size(small) rows(1)) name(Aaa,replace)
...
graph hbar Solar Wind Hydro_R HydroDispatch Nuc, over(labels, label(labsize(*0.85)) ///
		...
		legend(label(1 "Solar") label(2 "Wind") label(3 "Hydro-run") label(4 "Hydro-dispatch") label(5 "Nuclear") size(small) position(6) rows(2)) graphregion(margin(l+18)) name(bb,replace)
